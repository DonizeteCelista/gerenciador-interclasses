...
function devicePresenceId(){
  let id=localStorage.getItem("interclasses-device-id");
  if(!id){
    // Compatível também com WebViews e navegadores sem crypto.randomUUID().
    if(globalThis.crypto?.randomUUID){
      ...
      id=`disp-${Date.now().toString(36)}-${randomPart}`;
    }
    localStorage.setItem("interclasses-device-id",id);
  }
  return id;
}

function presenceCollection(){
  return collection(db,"escolas",state.school.id,"dispositivosAtivos");
}
...
function updateActiveDevicesWidget(){
  ...
  // Visível em todas as telas abertas pelo administrador daquela escola.
  const visible=!!state.admin&&!!state.school;
  widget.classList.toggle("hidden",!visible);
  if(!visible)return;

  // Uma margem maior evita perder aparelhos quando o navegador reduz timers em segundo plano.
  const cutoff=Date.now()-90000;
  const activeIds=new Set(
    (state.presenceDocs||[])
      .filter(item=>item.administrador!==true&&Number(item.ultimoSinal||0)>=cutoff)
      .map(item=>item.dispositivo||item.id||`${item.agente||""}-${item.ultimoSinal||0}`)
  );
  $("activeDevicesCount").textContent=String(activeIds.size);
}
...
async function sendPresenceHeartbeat(forceInactive=false){
  ...
  try{
    if(forceInactive){
      await deleteDoc(doc(db,"escolas",state.school.id,"dispositivosAtivos",dispositivo));
      return;
    }
    await setDoc(
      doc(db,"escolas",state.school.id,"dispositivosAtivos",dispositivo),
      {
        dispositivo,
        ultimoSinal:Date.now(),
        administrador:!!state.admin,
        pagina:state.screen||"",
...
function bindPresenceListeners(){
  ...
  // Mantém o aparelho contado enquanto a página estiver aberta, mesmo em outra aba.
  document.addEventListener("visibilitychange",()=>{
    if(state.school?.id&&document.visibilityState==="visible")sendPresenceHeartbeat();
  });

  window.addEventListener("pagehide",()=>{
    if(state.school?.id)sendPresenceHeartbeat(true);
  });
}
...
function startPresence(){
  ...
  sendPresenceHeartbeat();
  state.presenceHeartbeat=setInterval(()=>sendPresenceHeartbeat(),8000);
  state.presenceRefresh=setInterval(updateActiveDevicesWidget,2500);

  // Cada escola usa sua própria subcoleção de presença.
  state.presenceUnsub=onSnapshot(presenceCollection(),snap=>{
    state.presenceDocs=snap.docs.map(item=>({id:item.id,...item.data()}));
    updateActiveDevicesWidget();
  },error=>{
