...
function renderTab(tab,admin=false){const c=state.current,box=$("conteudoAluno");box.innerHTML="";updateClassificationNote(tab,c);if(tab==="proximos"||tab==="resultados"){
    ...
    list.forEach(game=>{
        ...
        :isLive
          ?game.live.tipo==="sets"
            ?`<div class="live-game-score">
                <strong>Set ${game.live.setAtual||1}: ${game.live.pontosCasa??0} × ${game.live.pontosFora??0}</strong>
                <small>Sets: ${game.live.placarCasa??0} × ${game.live.placarFora??0}</small>
              </div>`
      ...
      card.innerHTML=`
        ...
        ${isLive?'<div class="live-match-badge">● AO VIVO</div>':""}
...
function currentLiveGoals(){
  if(!state.currentGame||!["Futsal","Handebol"].includes(state.current?.esporte)){
    return[];
  }

  return [...document.querySelectorAll("#golsFutsal select")]
    .filter(select=>select.value)
    .map(select=>({
      jogadorId:select.value,
      lado:select.dataset.side
    }));
}

async function syncLiveScorers(){
  if(!state.current||!state.currentGame)return;
  if(!["Futsal","Handebol"].includes(state.current.esporte))return;

  clearTimeout(window.__liveScorerTimer);

  window.__liveScorerTimer=setTimeout(async()=>{
    try{
      ...
      game.liveGoals=currentLiveGoals();
      ...
      console.warn("Não foi possível atualizar a artilharia ao vivo:",error);
    }
  },350);
}

function bindLiveScorerControls(){
  const box=$("golsFutsal");
  if(!box||box.dataset.liveScorerBound==="1")return;

  box.dataset.liveScorerBound="1";
  box.addEventListener("change",event=>{
    if(event.target.matches("select[data-side]")){
      syncLiveScorers();
    }
  });
}
...
function currentSetLiveScore(){
  const sets=JSON.parse(JSON.stringify(state.setDraft||[]));
  const activeIndex=Math.max(0,sets.length-1);
  const activeSet=sets[activeIndex]||{casa:0,fora:0};
  ...
  sets.forEach(set=>{
    ...
    if(away>home)winsAway++;
  });

  return{
    tipo:"sets",
    sets,
    setAtual:activeIndex+1,
    pontosCasa:Number(activeSet.casa)||0,
    pontosFora:Number(activeSet.fora)||0,
    placarCasa:winsHome,
    placarFora:winsAway
  };
}
...
function scheduleLiveScoreSync(){
  ...
  liveScoreTimer=setTimeout(async()=>{
    try{
      ...
      const game=competition[located.list][located.i];
      const usesSets=(competition.tipoPlacar||"pontos")==="sets";
      ...
      console.warn("Não foi possível atualizar o placar ao vivo:",error);
    }
  },350);
}
...
function openResult(g){
  ...
  // Abre a tela primeiro. Recursos específicos de cada esporte
  // são carregados depois e não podem impedir o painel de aparecer.
  show("telaResultado");
  ...
  setText(
    "tituloJogoResultado",
    `Jogo ${stableGameNumber(state.current,g)} — ${g.casa||"A definir"} × ${g.fora||"A definir"}`
  );
  setText("timeCasaNome",g.casa||"A definir");
  setText("timeForaNome",g.fora||"A definir");
  ...
  try{
    ...
    console.warn("Detalhes do jogo:",error);
  }
  ...
  if(usesSets){
    ...
    try{
      ...
      console.error("Erro ao montar sets:",error);
      toast("O painel abriu, mas ocorreu um erro ao montar os sets.");
    }
  }else{
    state.setDraft=[];
  }
  ...
  try{
    ...
    console.warn("Placar ao vivo:",error);
  }
}
...
function renderSets(){
  ...
  state.setDraft.forEach((set,index)=>{
    ...
    card.innerHTML=`
      <div class="volley-set-title">
        <strong>Set ${index+1}</strong>
        ${state.setDraft.length>1?'<button class="set-delete" type="button">Excluir</button>':""}
      </div>

      <div class="volley-set-score">
        <div class="volley-set-side">
    ...
    card.querySelector("[data-home]").oninput=event=>{
      state.setDraft[index].casa=Math.max(0,Number(event.target.value)||0);
      scheduleLiveScoreSync();
    };
    card.querySelector("[data-away]").oninput=event=>{
      state.setDraft[index].fora=Math.max(0,Number(event.target.value)||0);
      scheduleLiveScoreSync();
    };
    ...
    const deleteButton=card.querySelector(".set-delete");
    if(deleteButton){
...
function recalcScorers(c){
  (c.participantes||[]).forEach(team=>
    (team.jogadores||[]).forEach(player=>player.gols=0)
  );

  allGames(c).forEach(game=>{
    const goals=game.encerrado
      ?(game.gols||[])
      :game.liveGoals||[];

    goals.forEach(goal=>{
      if(goal.jogadorId==="golContra")return;

      const team=(c.participantes||[]).find(participant=>
        (participant.jogadores||[]).some(player=>player.id===goal.jogadorId)
      );

      const player=(team?.jogadores||[]).find(item=>item.id===goal.jogadorId);

      if(player){
        player.gols=(player.gols||0)+1;
      }
    });
  });
}
