...
function updateActiveDevicesWidget(){
  ...
  // O contador aparece somente na página principal do painel do professor.
  const visible=!!state.admin&&!!state.school&&state.screen==="telaProfessor";
  widget.classList.toggle("hidden",!visible);
  if(!visible)return;

  const cutoff=Date.now()-35000;
  const total=(state.presenceDocs||[]).filter(item=>
    item.administrador!==true&&
    item.ativo!==false&&
    Number(item.ultimoSinal||0)>=cutoff
  ).length;
...
async function sendPresenceHeartbeat(forceInactive=false){
  ...
  const active=!forceInactive&&document.visibilityState!=="hidden";
  try{
    await setDoc(
      doc(db,"escolas",state.school.id,"dispositivosAtivos",devicePresenceId()),
      {
...
function bindPresenceListeners(){
  ...
  document.addEventListener("visibilitychange",()=>{
    if(!state.school?.id)return;
    sendPresenceHeartbeat(document.visibilityState==="hidden");
  });
...
function startPresence(){
  ...
  state.presenceHeartbeat=setInterval(()=>{
    if(document.visibilityState!=="hidden")sendPresenceHeartbeat();
  },10000);
  state.presenceRefresh=setInterval(updateActiveDevicesWidget,3000);

  // A coleção já fica dentro da escola atual, portanto cada escola tem sua própria contagem.
  state.presenceUnsub=onSnapshot(presenceCollection(),snap=>{
    state.presenceDocs=snap.docs.map(item=>item.data());
    updateActiveDevicesWidget();
  },error=>{
