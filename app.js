...
function listenStudent(){
  ...
  state.unsubStudent=onSnapshot(campsRef(),snap=>{
    ...
    const emptyNote=$("avisoPeriodoVazio");
    box.innerHTML="";
    emptyNote?.classList.add("hidden");

    periods.forEach(period=>{
      const competitions=camps.filter(c=>c.turno===period.key);
      ...
      button.onclick=()=>{
        if(!competitions.length){
          box.querySelectorAll(".compact-turn-card").forEach(item=>item.classList.remove("selected-empty-period"));
          button.classList.add("selected-empty-period");
          emptyNote?.classList.remove("hidden");
          return;
        }

        emptyNote?.classList.add("hidden");
        openStudentList(period.key,competitions);
      };
...
function filterStudentCompetitions(){
  const input=$("buscaCompeticaoAluno");
  ...
  if(!term){
    ...
    periods.forEach(period=>{
      const competitions=all.filter(c=>c.turno===period.key);
      const button=document.createElement("button");
      button.className="choice-card compact-turn-card";
      button.innerHTML=`
        <span>${period.icon}</span>
  ...
  const filtered=all.filter(c=>{
    const friendlyPeriod=
      c.turno==="Matutino"?"Manhã":
      c.turno==="Vespertino"?"Tarde":
      c.turno==="Noturno"?"Noite":
      "Outro";

    return norm(`${c.esporte} ${c.modalidade} ${c.turno} ${friendlyPeriod}`).includes(term);
  });
  ...
  filtered.forEach(c=>{
    ...
    button.innerHTML=`
      <span>${sportIcon(c.esporte)}</span>
      <strong>${esc(c.esporte)}</strong>
      <small>${esc(c.modalidade)} · ${esc(friendlyPeriod)}</small>
    `;
    button.onclick=()=>openCompetitionSafe(c,false);
    box.appendChild(button);
  });
}
...
function finishedSetScoresHtml(game){
  ...
  return `
    <div class="finished-set-scores" aria-label="Pontuação dos sets">
      ${sets.map(set=>`
        <div class="finished-set-line">
          <span>${Number(set.casa)||0}</span>
          <b>×</b>
          <span>${Number(set.fora)||0}</span>
        </div>
      `).join("")}
    </div>
  `;
