...
function renderTab(tab,admin=false){const c=state.current,box=$("conteudoAluno");box.innerHTML="";updateClassificationNote(tab,c);if(tab==="proximos"||tab==="resultados"){
  ...
  if(tab==="classificacao"){const t=$("tabClassificacao");if(t)t.innerHTML='Classificação <span class="tab-live-dot"></span>';return renderClassification(c,box);}if(tab==="eliminatorias")return renderKnockout(c,box);if(tab==="artilheiros")return renderScorers(c,box);if(tab==="podio")return renderPodium(c,box);if(tab==="resumoFinal")return renderFinalSummary(c,box);}
