...
function teamCode(competition,id,fallback="A definir"){
  ...
  if(!clean||clean==="A DEFINIR")return"A DEF";
...
function openResult(g){
  ...
  state.currentGame=g;
  $("tituloJogoResultado").textContent=`Jogo ${stableGameNumber(state.current,g)} — ${teamCode(state.current,g.casaId,g.casa)} × ${teamCode(state.current,g.foraId,g.fora)}`;
  $("timeCasaNome").textContent=teamCode(state.current,g.casaId,g.casa);
  $("timeForaNome").textContent=teamCode(state.current,g.foraId,g.fora);
