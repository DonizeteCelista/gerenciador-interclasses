...
const state={editPlayersOnly:false,screen:"telaBusca",school:null,admin:false,current:null,currentGame:null,wizard:newWizard(),unsubAdmin:null,unsubStudent:null,setDraft:[],editReturn:false,allChampionships:[],reorderDraft:null,reorderTab:"grupos"};
...
function renderSchoolList(){
  ...
  const arr=escolasDisponiveis.filter(school=>
    palavras.every(palavra=>norm(school.nome||school.nomeBusca||"").includes(palavra))
  );

  $("resultadosBusca").innerHTML="";

  if(!arr.length){
    $("resultadosBusca").innerHTML=`<div class="card muted">${termo?"Nenhuma escola encontrada. Tente outra parte do nome.":"Nenhuma escola cadastrada."}</div>`;
    return;
  }
  ...
  arr.forEach(school=>{
    ...
    button.innerHTML=`<span>🏫</span><div><strong>${esc(school.nome)}</strong><small>Toque para abrir</small></div>`;
    button.onclick=()=>openSchool(school);
    $("resultadosBusca").appendChild(button);
  });
}

async function searchSchools(forceReload=false){
  if(!forceReload&&escolasDisponiveis.length){
    renderSchoolList();
    return;
  }

  load(true);
  $("resultadosBusca").innerHTML='<div class="card muted">Carregando escolas...</div>';

  try{
    const snap=await getDocs(query(collection(db,"escolas"),orderBy("nomeBusca"),limit(300)));
    const now=Date.now();
    escolasDisponiveis=snap.docs
    ...
    console.error(error);
    $("resultadosBusca").innerHTML='<div class="card muted">Não foi possível carregar as escolas. Verifique sua conexão.</div>';
  }finally{load(false);}
}
...
function startStructuralEdit(section){
  ...
  if(section==="participantes"){
    ...
    $("textoParticipantes").textContent="Adicione ou exclua jogadores sem alterar os times e os jogos.";
    $("nomeParticipante").closest(".card").classList.add("hidden");
    $("btnAvancarRegras").textContent="Salvar jogadores";
...
$("btnBuscar").onclick=()=>searchSchools(false);$("campoBusca").oninput=renderSchoolList;$("campoBusca").onkeydown=e=>{if(e.key==="Enter")searchSchools(false);};$("btnAbrirCadastro").onclick=()=>show("telaCadastro");$("btnCancelarCadastro").onclick=()=>show("telaBusca");$("btnCriarEscola").onclick=createSchool;$("btnAbrirQREscola").onclick=openSchoolQRModal;$("btnFecharQREscola").onclick=closeSchoolQRModal;$("modalQREscola").onclick=e=>{if(e.target.id==="modalQREscola")closeSchoolQRModal();};$("btnProfessor").onclick=openTeacherArea;$("btnCancelarLogin").onclick=()=>show("telaEscola");$("btnEntrarProfessor").onclick=teacherLogin;$("btnNovoCampeonato").onclick=startWizard;$("btnSairPainel").onclick=()=>{state.admin=hasValidAdminSession(state.school);show("telaEscola");};$("btnTrocarEscola").onclick=()=>{clearAdminSession();localStorage.removeItem("ultimaEscola");state.school=null;state.admin=false;$("campoBusca").value="";show("telaBusca");searchSchools(true);};$("btnVoltar").onclick=goBack;$("btnConfirmarEsporteCustom").onclick=customSport;$("btnAdicionarParticipante").onclick=addParticipant;$("nomeParticipante").onkeydown=e=>{if(e.key==="Enter")addParticipant();};$("btnAvancarRegras").onclick=savePlayerRosters;$("btnAdicionarJogador").onclick=addPlayer;$("nomeJogador").onkeydown=e=>{if(e.key==="Enter")addPlayer();};$("btnConcluirJogadores").onclick=()=>{renderParticipants();show("telaParticipantes");};$("tipoPlacar").onchange=syncSetOptions;$("formatoSets").onchange=syncSetOptions;$("pontosSetNormal").onchange=syncSetOptions;$("pontosTieBreak").onchange=syncSetOptions;$("btnAvancarFormato").onclick=openFormat;document.querySelectorAll(".format-card").forEach(b=>b.onclick=()=>chooseFormat(b.dataset.formato,b));$("quantidadeGrupos").onchange=updateGroupRules;
...
async function savePlayerRosters(){
  ...
  try{
    ...
    await updateDoc(doc(db,"escolas",state.school.id,"campeonatos",state.current.id),{participantes,atualizadoEm:serverTimestamp()});
    state.current={...state.current,participantes};
  ...
  }catch(error){console.error(error);toast("Não foi possível salvar os jogadores.");}
  finally{load(false);}
}
...
const startupUrl=new URL(location.href);startupUrl.searchParams.delete("campeonato");const id=startupUrl.searchParams.get("escola");if(id)openSchoolById(id);else{$("campoBusca").value="";show("telaBusca");searchSchools(true);}
