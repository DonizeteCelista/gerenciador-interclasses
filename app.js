...
function normalizeTeamCodeText(value){
  return String(value||"")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g,"")
    .toUpperCase()
...
function teamCodeCandidates(name){
  const clean=normalizeTeamCodeText(name);
  if(!clean)return["---"];

  // Turmas numéricas continuam iguais: 204, 101 etc.
  if(/^\d+$/.test(clean)){
  ...
  const add=value=>{
    const code=String(value||"").replace(/[^A-Z0-9]/g,"").slice(0,3);
  ...
  if(words.length>1){
    ...
    add((words[0].slice(0,2)+(words[1]?.[0]||"")));
    add((words[0]?.[0]||"")+(words[1]?.slice(0,2)||""));
    add((words[0]?.[0]||"")+(words[1]?.[0]||"")+(words.at(-1)?.[0]||""));
  }

  // Outras combinações preservam letras do nome e resolvem duplicidades.
  for(let a=0;a<letters.length;a++){
    for(let b=a+1;b<letters.length;b++){
      for(let c=b+1;c<letters.length;c++){
        add(letters[a]+letters[b]+letters[c]);
      }
    }
  }

  add((letters+"XXX").slice(0,3));
  return candidates.length?candidates:["---"];
}

function ensureTeamCodes(competition){
  const used=new Set();

  (competition?.participantes||[]).forEach((participant,index)=>{
    const saved=normalizeTeamCodeText(participant.sigla||"").replace(/ /g,"").slice(0,3);

    if(saved.length===3&&!used.has(saved)){
      participant.sigla=saved;
      used.add(saved);
      return;
    }

    const candidates=teamCodeCandidates(participant.nome);
    let code=candidates.find(candidate=>!used.has(candidate));

    if(!code){
      const base=(candidates[0]||"TIM").slice(0,2);
      const chars="ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
      code=[...chars].map(char=>base+char).find(candidate=>!used.has(candidate));
    }

    if(!code)code=`T${String(index+1).padStart(2,"0")}`.slice(0,3);

    participant.sigla=code;
    used.add(code);
  });
...
function isIndividualCompetition(competition){
  const sport=normalizeTeamCodeText(competition?.esporte||"");
  const explicit=normalizeTeamCodeText(
    competition?.tipoParticipante||
    competition?.participanteTipo||
    competition?.categoriaParticipante||
    ""
  );

  if(["INDIVIDUAL","JOGADOR","JOGADORES"].includes(explicit))return true;
  ...
  return[
    ...
    "BADMINTON",
    "ATLETISMO"
  ].some(name=>sport.includes(name));
}

function teamCode(competition,id,fallback="A definir"){
  ensureTeamCodes(competition);

  const participant=(competition?.participantes||[]).find(item=>
    (id!=null&&String(item.id)===String(id))||
    normalizeTeamCodeText(item.nome)===normalizeTeamCodeText(fallback)
  );

  if(isIndividualCompetition(competition)){
    const fullName=participant?.nome||fallback;
    const cleanFull=String(fullName||"").trim();
    return !cleanFull||normalizeTeamCodeText(cleanFull)==="A DEFINIR"
      ?"???"
      :cleanFull;
  }

  if(participant?.sigla)return participant.sigla;

  const clean=normalizeTeamCodeText(fallback);
  if(!clean||clean==="A DEFINIR")return"???";
  if(/^\d+$/.test(clean))return clean.slice(0,3);
  return (clean.replace(/ /g,"")+"XXX").slice(0,3);
}
...
function normalizeCompetition(c){
  ...
  return ensureTeamCodes(normalized);
}
    ...
    list.forEach(game=>{
          ...
          ?game.live.tipo==="sets"
            ?`<div class="live-game-score live-set-score">
                ...
                <div class="live-set-line">
                  <strong class="live-set-team">${esc(game.casa||"Time A")}</strong>
                  <b>${game.live.pontosCasa??0} × ${game.live.pontosFora??0}</b>
                  <strong class="live-set-team">${esc(game.fora||"Time B")}</strong>
                </div>
                <small>● AO VIVO</small>
              </div>`
      ...
      card.innerHTML=`
        ...
        <div class="horizontal-match">
          <div class="horizontal-team">${esc(teamCode(c,game.casaId,game.casa||"A definir"))}</div>
          ${score}
          <div class="horizontal-team">${esc(teamCode(c,game.foraId,game.fora||"A definir"))}</div>
        </div>
      ...
      <tbody>
        ${rows.map((row,index)=>`
          <tr class="${index<qualified?"qualified":""}">
            <td>${index+1}º ${row.aoVivo?'<span class="live-dot" title="Jogando agora"></span>':""}${esc(teamCode(c,row.id,row.nome))}${index<qualified?' <span title="Classificado">✓</span>':""}</td>
            <td>${row.pts}</td>
            <td>${row.j}</td>
            <td>${row.gp-row.gc}</td>
            <td>${row.gp}</td>
      ...
      .forEach(g=>{
        ...
        card.innerHTML=`
          ...
          <div class="bracket-team ${winner?.id===g.casaId?"winner":g.encerrado&&winner&&!direct?"loser":""}">
            <span>${esc(teamCode(c,g.casaId,g.casa))}</span><strong>${!direct&&g.encerrado?displayedSetScore(g,"casa"):""}</strong>
          </div>
          <div class="bracket-team ${winner?.id===g.foraId?"winner":g.encerrado&&winner&&!direct?"loser":""}">
            <span>${esc(teamCode(c,g.foraId,g.fora))}</span><strong>${!direct&&g.encerrado?displayedSetScore(g,"fora"):""}</strong>
          </div>
...
function renderScorers(c,box){const a=[];c.participantes.forEach(p=>(p.jogadores||[]).forEach(j=>a.push({...j,time:teamCode(c,p.id,p.nome)})));a.sort((x,y)=>(y.gols||0)-(x.gols||0));if(!a.length){box.innerHTML='<div class="card muted">Nenhum jogador cadastrado.</div>';return;}a.forEach((j,i)=>{const e=document.createElement("div");e.className="admin-item";e.innerHTML=`<div><strong>${i+1}º ${esc(j.nome)}</strong><small>${esc(j.time)}</small></div><strong>${j.gols||0} gols</strong>`;box.appendChild(e);});}
function renderPodium(c,box){if(!c.podio){box.innerHTML='<div class="card muted">O pódio aparecerá quando o campeonato for finalizado.</div>';return;}box.innerHTML=`<div class="podium"><div class="podium-place second"><span>🥈</span><strong>${esc(teamCode(c,null,c.podio.segundo||"—"))}</strong><small>2º lugar</small></div><div class="podium-place first"><span>🥇</span><strong>${esc(teamCode(c,null,c.podio.primeiro||"—"))}</strong><small>Campeão</small></div><div class="podium-place third"><span>🥉</span><strong>${esc(teamCode(c,null,c.podio.terceiro||"—"))}</strong><small>3º lugar</small></div></div>`;}
function locateGame(c,id){
  const target=String(id);

  let index=(c.jogos||[]).findIndex(game=>String(game.id)===target);
  if(index>=0)return{list:"jogos",i:index};

  index=(c.eliminatorias||[]).findIndex(game=>String(game.id)===target);
  return index>=0?{list:"eliminatorias",i:index}:null;
};i=(c.eliminatorias||[]).findIndex(j=>j.id===id);return i>=0?{list:"eliminatorias",i}:null;}
...
function loadPenaltyFields(g){
  $("vencedorPenaltis").innerHTML=`
    ...
    <option value="${g.casaId||""}">${esc(teamCode(c,g.casaId,g.casa))}</option>
    <option value="${g.foraId||""}">${esc(teamCode(c,g.foraId,g.fora))}</option>
  `;
...
function openResult(g){
  if(!state.current||!g){
    return toast("Partida não encontrada.");
  }
  ...
  // A tela abre antes dos recursos complementares.
  // Assim, um erro em sets, gols, pênaltis ou ao vivo não bloqueia o painel.
  show("telaResultado");
  ...
  const homeName=teamCode(state.current,g.casaId,g.casa||"A definir");
  const awayName=teamCode(state.current,g.foraId,g.fora||"A definir");
  const usesSets=(state.current.tipoPlacar||"pontos")==="sets";
  const supportsGoals=["Futsal","Handebol"].includes(state.current.esporte);

  setText(
    "tituloJogoResultado",
    `Jogo ${stableGameNumber(state.current,g)} — ${homeName} × ${awayName}`
  );
  setText("timeCasaNome",homeName);
  setText("timeForaNome",awayName);

  setValue("dataJogo",g.data||"");
  setValue("horaJogo",g.hora||"");
  setValue("localJogo",g.local||"");
  setValue("situacaoJogo",g.situacao||"normal");
  setValue("placarCasa",g.placarCasa??0);
  setValue("placarFora",g.placarFora??0);

  const hasDetails=!!(g.data||g.hora||g.local);
  toggleHidden("detalhesJogoBox",!hasDetails);
  toggleHidden("placarSimplesBox",usesSets);
  toggleHidden("placarSetsBox",!usesSets);
  toggleHidden("golsFutsalBox",!supportsGoals);

  try{
    if(typeof updateGameDetailsToggle==="function")updateGameDetailsToggle();
  }catch(error){
    console.warn("Detalhes da partida:",error);
  }

  try{
    if(typeof loadPenaltyFields==="function")loadPenaltyFields(g);
    if(typeof updatePenaltyBox==="function")updatePenaltyBox();
  }catch(error){
    console.warn("Pênaltis:",error);
    toggleHidden("penaltisBox",true);
  }

  if(usesSets){
    state.setDraft=JSON.parse(JSON.stringify(g.sets||[]));
    if(!state.setDraft.length)state.setDraft=[{casa:0,fora:0}];

    try{
      if(typeof renderSets==="function")renderSets();
    }catch(error){
      console.error("Sets:",error);
      toast("O painel abriu, mas houve um erro ao montar os sets.");
    }
  }else{
    state.setDraft=[];
  }
  ...
  if(supportsGoals){
    try{
      ...
      if(typeof updateGoalInputs==="function")updateGoalInputs();
      if(typeof bindLiveScorerControls==="function")bindLiveScorerControls();
    }catch(error){
      console.warn("Autores dos gols:",error);
      toggleHidden("golsFutsalBox",true);
    }
  }

  try{
    if(typeof updateClassificationPreview==="function")updateClassificationPreview();
  }catch(error){
    console.warn("Prévia/caminho da partida:",error);
  }

  try{
    if(typeof bindLiveGameControls==="function")bindLiveGameControls();
    if(typeof scheduleLiveGameSync==="function")scheduleLiveGameSync();
    if(typeof bindLiveScoreControls==="function")bindLiveScoreControls();
    if(typeof scheduleLiveScoreSync==="function")scheduleLiveScoreSync();
  }catch(error){
    console.warn("Jogo ao vivo:",error);
  }
}
...
function updateClassificationPreview(){
  ...
  box.innerHTML=`
    ...
    <div class="classification-preview-grid classification-preview-header">
      <span>Time</span><span>P</span><span>J</span><span>SG</span><span>GP</span>
    </div>
    ${preview.rows.map((row,index)=>`
      <div class="classification-preview-grid">
        <strong>${index+1}º ${esc(teamCode(c,row.id,row.nome))}</strong>
        <span>${row.pts}</span>
        <span>${row.j}</span>
        <span>${row.gp-row.gc}</span>
        <span>${row.gp}</span>
      </div>
    `).join("")}
  `;
    ...
    card.innerHTML=`
      ...
      <div class="volley-score-board">
        <div class="volley-score-side">
          <strong class="volley-team-name">${esc(teamCode(state.current,state.currentGame?.casaId,state.currentGame?.casa||"Time A"))}</strong>
        ...
        <div class="volley-score-side">
          <strong class="volley-team-name">${esc(teamCode(state.current,state.currentGame?.foraId,state.currentGame?.fora||"Time B"))}</strong>
          <div class="score-control compact-score">
...
function renderGoalInputs(){
  ...
  box.innerHTML=`
    <label>Gols de ${esc(teamCode(c,g.casaId,g.casa))}</label>
    <div id="golsCasaInputs"></div>
    <label>Gols de ${esc(teamCode(c,g.foraId,g.fora))}</label>
...
function renderAgendaPreview(c){
  ...
  games.slice(0,20).forEach(g=>{
    ...
    e.innerHTML=`<div class="match-time">${g.hora||"--:--"}</div><div><strong>${esc(teamCode(c,g.casaId,g.casa))} × ${esc(teamCode(c,g.foraId,g.fora))}</strong><small>${g.data?new Date(g.data+"T12:00:00").toLocaleDateString("pt-BR"):"Sem data"} · ${esc(g.local||"Local não definido")}</small></div>`;
    box.appendChild(e);
  });
}
...
function renderFinalSummary(c,box){
  ...
  box.innerHTML=`
    <div class="summary-final">
      ...
      <div class="metric-list">
        <div class="metric"><strong>${esc(teamCode(c,null,c.podio.segundo||"—"))}</strong><small>Vice-campeão</small></div>
        <div class="metric"><strong>${esc(teamCode(c,null,c.podio.terceiro||"—"))}</strong><small>3º lugar</small></div>
