...
const state={screen:"telaBusca",school:null,admin:false,current:null,currentGame:null,wizard:newWizard(),unsubAdmin:null,unsubStudent:null,setDraft:[],editReturn:false,allChampionships:[],reorderDraft:null,reorderTab:"grupos",presenceUnsub:null,presenceHeartbeat:null,presenceRefresh:null,presenceDocs:[]};
...
function updateActiveDevicesWidget(){
  ...
  const total=(state.presenceDocs||[]).filter(item=>Number(item.ultimoSinal||0)>=cutoff).length;
...
function startPresence(){
  ...
  sendPresenceHeartbeat();
  state.presenceHeartbeat=setInterval(sendPresenceHeartbeat,15000);
  state.presenceRefresh=setInterval(updateActiveDevicesWidget,5000);
