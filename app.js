...
const state={editPlayersOnly:false,screen:"telaBusca",school:null,admin:false,current:null,currentGame:null,wizard:newWizard(),unsubAdmin:null,unsubStudent:null,setDraft:[],editReturn:false,allChampionships:[],reorderDraft:null,reorderTab:"grupos",basketHistory:[],basketLockUntil:0,basketLockTimer:null,schoolDirectory:[],schoolDirectoryReady:false,schoolDirectoryUnsub:null};
...
function show(id){
  ...
  state.screen=id;
  $("btnVoltar").classList.toggle("hidden",id==="telaBusca");
  scrollTo({top:0,behavior:"smooth"});
  if(id==="telaBusca"){
    clearTimeout(window.__schoolInitialLoadTimer);
    window.__schoolInitialLoadTimer=setTimeout(()=>searchSchools(true),80);
  }
}
...
function activeSchoolsFromSnapshot(snap){
  const now=Date.now();
  return snap.docs
...
function renderSchoolDirectory(){
  ...
  const arr=(state.schoolDirectory||[]).filter(s=>
    palavras.length===0||palavras.every(word=>norm(`${s.nome||""} ${s.nomeBusca||""}`).includes(word))
  );
  results.innerHTML="";
  if(!arr.length){
    results.innerHTML=`<div class="card muted">${state.schoolDirectoryReady?(palavras.length?"Nenhuma escola encontrada.":"Nenhuma escola cadastrada."):"Carregando escolas..."}</div>`;
    return;
  }
  arr.forEach(school=>{
...
function startSchoolDirectory(){
  if(state.schoolDirectoryUnsub)return;
  const results=$("resultadosBusca");
  if(results&&!results.children.length)results.innerHTML='<div class="card muted">Carregando escolas...</div>';
  try{
    state.schoolDirectoryUnsub=onSnapshot(collection(db,"escolas"),snap=>{
      state.schoolDirectory=activeSchoolsFromSnapshot(snap);
      state.schoolDirectoryReady=true;
      if(state.screen==="telaBusca")renderSchoolDirectory();
    },async error=>{
      console.error("Erro ao acompanhar escolas:",error);
      state.schoolDirectoryUnsub=null;
      try{
        ...
        console.error("Erro ao carregar escolas:",fallbackError);
        if(results)results.innerHTML='<div class="card muted">Não foi possível carregar as escolas. Verifique a conexão e tente novamente.</div>';
        setTimeout(startSchoolDirectory,1800);
      }
    });
  }catch(error){
    console.error("Erro ao iniciar lista de escolas:",error);
    setTimeout(startSchoolDirectory,1200);
  }
}

async function searchSchools(){
  startSchoolDirectory();
  renderSchoolDirectory();
}

function filterSchoolsInRealTime(){
  clearTimeout(schoolSearchTimer);
  schoolSearchTimer=setTimeout(renderSchoolDirectory,80);
}
...
function openRules(){
  ...
  state.wizard.tipoPlacar = custom
    ? (["gols","sets","tempo"].includes(state.wizard.tipoPlacar)?state.wizard.tipoPlacar:"gols")
  ...
  if(custom){
    $("regraAutomaticaResumo").textContent = "Como este é um esporte personalizado, escolha entre gols, pontos por sets ou pontos por tempo.";
  ...
  }else if(state.wizard.tipoPlacar === "tempo"){
    $("regraAutomaticaResumo").textContent = `${state.wizard.esporte}: resultado por pontos e controle de tempo, como no basquete.`;
  }else if(state.wizard.tipoPlacar === "gols"){
    $("regraAutomaticaResumo").textContent = `${state.wizard.esporte}: resultado por gols, com registro dos autores.`;
  }else{
    $("regraAutomaticaResumo").textContent = `${state.wizard.esporte}: resultado por pontos.`;
  }
...
function directBronzeWinner(c){
  ...
  if(c.regraClassificacao==="lideresFinal"&&c.grupos.length===2){
    ...
    return [secondA,secondB].filter(Boolean).sort((a,b)=>compareStandingRows(c,a,b))[0]||null;
  }
...
function classificationCriteriaText(c){
  const base=c.tipoPlacar==="sets"
    ...
    :usesBasketballScoring(c)
      ?"Critérios de classificação: pontos, jogos, saldo de pontos, pontos marcados e ordem alfabética."
      :"Critérios de desempate: pontos, saldo de gols, gols pró e ordem alfabética.";
...
function classificationLegend(c){
  return c.tipoPlacar==="sets"
    ...
    :usesBasketballScoring(c)
      ?"P = Pontos · J = Jogos · SP = Saldo de pontos · PM = Pontos marcados"
      :"P = Pontos · J = Jogos · SG = Saldo de gols · GP = Gols pró";
}
...
function renderClassification(c,box){
  ...
  const usesSets=c.tipoPlacar==="sets";
  const usesBasketball=usesBasketballScoring(c);
  const col3=usesSets?"SV":usesBasketball?"SP":"SG";
  const col4=usesSets?"PM":usesBasketball?"PM":"GP";
...
function usesBasketballScoring(c){
  return ["Basquete","Basquete 3x3"].includes(c?.esporte)||c?.tipoPlacar==="tempo";
}

function isBasketballCompetition(){
  return usesBasketballScoring(state.current);
}

function usesGoalScorers(competition=state.current){
  return ["Futsal","Handebol"].includes(competition?.esporte)||competition?.tipoPlacar==="gols";
}
...
function renderBasketHistory(){
  ...
  box.innerHTML=history.slice().reverse().map(item=>{
    ...
    const action=points<0?`teve ${Math.abs(points)} ponto removido`:`marcou +${points}`;
    return `<div class="basket-history-item"><span><strong>${esc(basketSideName(item.lado))}</strong> ${action}</span><small>${esc(time)}</small></div>`;
  }).join("");
}

function updateBasketLockUI(){
  const buttons=["basketCasa1","basketCasa2","basketCasa3","basketFora1","basketFora2","basketFora3"].map($).filter(Boolean);
...
function removeBasketPoint(side){
  ...
  if(current<=0)return toast("O placar já está zerado.");
  const now=Date.now();
  input.value=current-1;
  state.basketHistory.push({lado:side,pontos:-1,timestamp:now,horario:new Date(now).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit",second:"2-digit"})});
  renderBasketHistory();
  updatePenaltyBox();
...
function addBasketPoints(side,points){
  ...
  state.basketLockUntil=now+5000;
  renderBasketHistory();
  updateBasketLockUI();
  updatePenaltyBox();
  updateClassificationPreview();
...
function openResult(g){
  ...
  $("golsFutsalBox").classList.toggle("hidden",!usesGoalScorers(state.current));
  ...
  state.basketLockUntil=basketball&&lastBasketPoint?.timestamp?Number(lastBasketPoint.timestamp)+5000:0;
  renderBasketHistory();
  updateBasketLockUI();
...
function updateClassificationPreview(){
  ...
  box.innerHTML=`
    ...
    <div class="classification-preview-grid classification-preview-header">
      ...
      <span>${state.current.tipoPlacar==="sets"?"SV":usesBasketballScoring(state.current)?"SP":"SG"}</span>
      <span>${state.current.tipoPlacar==="sets"?"PM":usesBasketballScoring(state.current)?"PM":"GP"}</span>
    </div>
...
function updateGoalInputs(){
  if(!usesGoalScorers(state.current))return;
...
async function saveGame(clear=false){const c=JSON.parse(JSON.stringify(state.current)),loc=locateGame(c,state.currentGame.id);if(!loc)return toast("Jogo não encontrado.");const g=c[loc.list][loc.i];if(clear){Object.assign(g,{encerrado:false,placarCasa:null,placarFora:null,situacao:"normal",gols:[],sets:[],pontuacoesBasquete:[],vencedorPenaltisId:null,vencedorPenaltisNome:"",penaltisCasa:null,penaltisFora:null});}else{g.data=$("dataJogo").value;g.hora=$("horaJogo").value;g.local=$("localJogo").value.trim();g.situacao=$("situacaoJogo").value;if(g.situacao==="adiado"||g.situacao==="cancelado"){g.encerrado=g.situacao==="cancelado";g.placarCasa=null;g.placarFora=null;}else if(c.tipoPlacar==="sets"){if(g.situacao==="normal"){const v=validateSets(c);if(!v.ok)return toast(v.msg);g.sets=state.setDraft;g.placarCasa=v.winsCasa;g.placarFora=v.winsFora;}else{g.sets=[];g.placarCasa=g.situacao==="woCasa"?3:0;g.placarFora=g.situacao==="woFora"?3:0;}g.encerrado=true;
  ...
  g.penaltisCasa=null;
  g.penaltisFora=null;
}g.placarCasa=pc;g.placarFora=pf;g.pontuacoesBasquete=isBasketballCompetition()?JSON.parse(JSON.stringify(state.basketHistory||[])):[];g.encerrado=true;g.gols=[];if(usesGoalScorers(c)&&g.situacao==="normal")document.querySelectorAll("#golsFutsal select").forEach(s=>{if(s.value)g.gols.push({jogadorId:s.value,lado:s.dataset.side});});}}
if(g.live)delete g.live;
  ...
  box.innerHTML=`
    <div class="summary-final">
      ...
      <div class="metric-list">
        ...
        <div class="metric"><strong>${scorers[0]?esc(scorers[0].nome):"—"}</strong><small>${usesGoalScorers(c)?"Artilheiro":"Destaque"}</small></div>
      </div>
    </div>`;
}
...
startupUrl.searchParams.delete("campeonato");
const id=startupUrl.searchParams.get("escola");
if(id){
  openSchoolById(id);
}else{
  show("telaBusca");
  setTimeout(()=>searchSchools(true),250);
}
window.addEventListener("pageshow",()=>{
  if(state.screen==="telaBusca"&&!new URL(location.href).searchParams.get("escola"))setTimeout(()=>searchSchools(true),120);
});
