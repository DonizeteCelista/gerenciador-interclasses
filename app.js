...
async function searchSchools(showAll=false){
  ...
  if(!showAll&&palavras.join("").length<2){
  ...
  try{
    // Carrega as escolas sem depender do campo nomeBusca existir em todos os documentos.
    // A ordenação é feita no próprio aplicativo para garantir que a lista inicial sempre apareça.
    ...
    const arr=snap.docs
      ...
      .filter(s=>showAll||palavras.every(palavra=>norm(s.nome||s.nomeBusca||"").includes(palavra)))
      .sort((a,b)=>String(a.nome||"").localeCompare(String(b.nome||""),"pt-BR",{sensitivity:"base"}))
      .slice(0,300);

    if(!arr.length){
      $("resultadosBusca").innerHTML=`<div class="card muted">${showAll?"Nenhuma escola cadastrada.":"Nenhuma escola encontrada. Tente uma palavra ou parte do nome."}</div>`;
      return;
    }
    ...
    console.error(error);
    $("resultadosBusca").innerHTML=`<div class="card muted">Não foi possível carregar as escolas. Verifique a conexão e tente novamente.</div>`;
    toast("Erro ao carregar as escolas.");
  }finally{load(false);}
}
