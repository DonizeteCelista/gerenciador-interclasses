...
function renderTab(tab,admin=false){const c=state.current,box=$("conteudoAluno");box.innerHTML="";updateClassificationNote(tab,c);if(tab==="proximos"||tab==="resultados"){
    ...
    list.forEach(game=>{
      ...
      const score=(game.encerrado||game.emAndamento)
        ?`<div class="simple-score ${game.emAndamento&&!game.encerrado?"live-score":""}">
            ${game.placarCasa??0} × ${game.placarFora??0}
          </div>`
      ...
      card.innerHTML=`
        ...
        ${game.emAndamento&&!game.encerrado
          ?'<div class="live-match-badge">● AO VIVO</div>'
          :""}
      ...
      if(admin){
        ...
        if(tab==="resultados"){
          ...
          button.textContent=game.emAndamento?"Atualizar placar":"Lançar resultado";
          button.onclick=event=>{
            event.stopPropagation();
            openResult(game);
          };
...
function standings(c,g){
  const table=g.participantes.map(team=>({
    id:team.id,
    nome:team.nome,
    pts:0,
    j:0,
  ...
  (c.jogos||[])
    .filter(game=>
      ...
      (game.encerrado||game.emAndamento)&&
    ...
    .forEach(game=>{
      ...
      const homeScore=Number(game.placarCasa)||0;
      const awayScore=Number(game.placarFora)||0;
...
function renderClassification(c,box){
  ...
  const hasLive=(c.jogos||[]).some(game=>game.emAndamento&&!game.encerrado);

  if(hasLive){
    const liveNote=document.createElement("div");
    liveNote.className="live-standings-note";
    liveNote.innerHTML='<strong>● CLASSIFICAÇÃO AO VIVO</strong><span>Há partida em andamento. As posições são provisórias.</span>';
      ...
      <tbody>
        ${rows.map((row,index)=>`
          <tr class="${index<qualified?"qualified":""}">
            <td>${index+1}º ${esc(row.nome)}${index<qualified?' <span title="Classificado">✓</span>':""}</td>
            <td>${row.pts}</td>
            <td>${row.j}</td>
            <td>${row.gp-row.gc}</td>
            <td>${row.gp}</td>
...
async function saveGame(clear=false,finalize=true){
  const c=JSON.parse(JSON.stringify(state.current));
  const loc=locateGame(c,state.currentGame.id);

  if(!loc)return toast("Jogo não encontrado.");

  const g=c[loc.list][loc.i];
  ...
  if(clear){
    Object.assign(g,{
      ...
      gols:[],
      sets:[],
      vencedorPenaltisId:null,
      vencedorPenaltisNome:"",
      penaltisCasa:null,
      penaltisFora:null
    });
  }else{
    g.data=$("dataJogo").value;
    g.hora=$("horaJogo").value;
    g.local=$("localJogo").value.trim();
    g.situacao=$("situacaoJogo").value;

    if(g.situacao==="adiado"||g.situacao==="cancelado"){
      g.encerrado=g.situacao==="cancelado";
      g.emAndamento=false;
      g.placarCasa=null;
      g.placarFora=null;
      g.gols=[];
      g.sets=[];
    }else if(c.tipoPlacar==="sets"){
      if(g.situacao==="normal"){
        if(finalize){
          const validation=validateSets(c);
          if(!validation.ok)return toast(validation.msg);
          g.placarCasa=validation.winsCasa;
          g.placarFora=validation.winsFora;
        }else{
          g.placarCasa=(state.setDraft||[]).filter(set=>set.casa>set.fora).length;
          g.placarFora=(state.setDraft||[]).filter(set=>set.fora>set.casa).length;
        }

        g.sets=JSON.parse(JSON.stringify(state.setDraft||[]));
      }else{
        g.sets=[];
        g.placarCasa=g.situacao==="woCasa"?3:0;
        g.placarFora=g.situacao==="woFora"?3:0;
      }

      g.encerrado=finalize;
      g.emAndamento=!finalize;
      g.gols=[];
    }else{
      let homeScore=Math.max(0,Number($("placarCasa").value)||0);
      let awayScore=Math.max(0,Number($("placarFora").value)||0);

      if(g.situacao==="woCasa"){homeScore=3;awayScore=0;}
      if(g.situacao==="woFora"){homeScore=0;awayScore=3;}
      if(g.situacao==="woDuplo"){homeScore=0;awayScore=0;}

      if(finalize&&loc.list==="eliminatorias"&&g.situacao==="normal"&&homeScore===awayScore){
        const winnerId=$("vencedorPenaltis").value;
        const penaltyHome=Math.max(0,Number($("penaltisCasa").value)||0);
        const penaltyAway=Math.max(0,Number($("penaltisFora").value)||0);

        if(!winnerId)return toast("Selecione quem venceu nos pênaltis.");
        if(penaltyHome===penaltyAway)return toast("O placar dos pênaltis precisa ter um vencedor.");

        if(
          (winnerId===g.casaId&&penaltyHome<penaltyAway)||
          (winnerId===g.foraId&&penaltyAway<penaltyHome)
        ){
        ...
        g.penaltisCasa=penaltyHome;
        g.penaltisFora=penaltyAway;
      }else{
      ...
      g.placarCasa=homeScore;
      g.placarFora=awayScore;
      g.encerrado=finalize;
      g.emAndamento=!finalize;
      g.gols=[];

      if(["Futsal","Handebol"].includes(c.esporte)&&g.situacao==="normal"){
        document.querySelectorAll("#golsFutsal select").forEach(select=>{
          if(select.value){
            g.gols.push({
  ...
  try{
    ...
    toast(
      clear
        ...
        :finalize
          ?"Partida finalizada."
          :"Andamento salvo. Classificação atualizada ao vivo."
    );
