...
function renderSchoolList(){
  ...
  const lista=escolasDisponiveis
    .filter(school=>palavras.every(palavra=>norm(school.nome||school.nomeBusca||"").includes(palavra)));

  $("resultadosBusca").innerHTML="";

  if(!lista.length){
    $("resultadosBusca").innerHTML=`<div class="card muted">${termo?"Nenhuma escola encontrada.":"Nenhuma escola cadastrada."}</div>`;
    return;
  }

  lista.forEach(school=>{
...
const startupUrl=new URL(location.href);startupUrl.searchParams.delete("campeonato");const id=startupUrl.searchParams.get("escola");if(id)openSchoolById(id);else{localStorage.removeItem("ultimaEscola");show("telaBusca");searchSchools(true);}
