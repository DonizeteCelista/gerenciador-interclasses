...
async function searchSchools(showAll=false){
  ...
  try{
    ...
    console.error(error);
    if(!showAll)toast("Erro ao pesquisar.");
  }finally{load(false);}
}
...
const startupUrl=new URL(location.href);startupUrl.searchParams.delete("campeonato");const id=startupUrl.searchParams.get("escola");if(id)openSchoolById(id);else{const last=localStorage.getItem("ultimaEscola");if(last)try{openSchool(JSON.parse(last));}catch{localStorage.removeItem("ultimaEscola");searchSchools(true);}else searchSchools(true);}
