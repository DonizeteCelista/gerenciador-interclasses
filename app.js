...
function locateGame(c,id){
  let index=(c.jogos||[]).findIndex(game=>game.id===id);

  if(index>=0){
    return{list:"jogos",i:index};
  }

  index=(c.eliminatorias||[]).findIndex(game=>game.id===id);

  return index>=0
    ?{list:"eliminatorias",i:index}
    :null;
}
...
function updateClassificationNote(tab,c){
  ...
  if(visible){
    note.textContent=classificationCriteriaText(c);
  }
}
...
function openResult(g){
  try{
    if(!state.current||!g)return toast("Partida não encontrada.");
  ...
  updateGoalInputs();
    ...
    console.error("Erro ao abrir editor de resultado:",error);
    toast("Não foi possível abrir o placar. Atualize a página e tente novamente.");
  }
}
...
function updateGoalInputs(){
  ...
  if(!$("golsFutsal")||!$("golsCasaInputs")||!$("golsForaInputs"))return;
...
function setResultButtonsDisabled(disabled){
  ["btnSalvarResultado","btnLimparResultado","btnInverterLados",
   "maisCasa","menosCasa","maisFora","menosFora"].forEach(id=>{
    const element=$(id);
    if(element)element.disabled=disabled;
  });
}

async function saveGame(clear=false){
  ...
  if(!current||!currentGame){
    return toast("Partida não encontrada. Volte e abra o jogo novamente.");
  }

  const c=JSON.parse(JSON.stringify(current));
  const loc=locateGame(c,currentGame.id);

  if(!loc){
    return toast("Jogo não encontrado. Atualize a competição.");
  }
  ...
  try{
    ...
    if(clear){
      ...
      g.situacao=$("situacaoJogo")?.value||"normal";
      ...
      if(g.situacao==="adiado"||g.situacao==="cancelado"){
        ...
        let home=Math.max(0,Number($("placarCasa")?.value)||0);
        let away=Math.max(0,Number($("placarFora")?.value)||0);

        if(g.situacao==="woCasa"){home=3;away=0;}
        if(g.situacao==="woFora"){home=0;away=3;}
        if(g.situacao==="woDuplo"){home=0;away=0;}

        if(loc.list==="eliminatorias"&&g.situacao==="normal"&&home===away){
          const winnerId=$("vencedorPenaltis")?.value||"";
          const penaltiesHome=Math.max(0,Number($("penaltisCasa")?.value)||0);
          const penaltiesAway=Math.max(0,Number($("penaltisFora")?.value)||0);

          if(!winnerId){
            return toast("Selecione quem venceu nos pênaltis.");
          }

          if(penaltiesHome===penaltiesAway){
            return toast("O placar dos pênaltis precisa ter um vencedor.");
          }

          if(
            (winnerId===g.casaId&&penaltiesHome<penaltiesAway)||
            (winnerId===g.foraId&&penaltiesAway<penaltiesHome)
          ){
          ...
          g.penaltisCasa=penaltiesHome;
          g.penaltisFora=penaltiesAway;
        }else{
    ...
    if(loc.list==="eliminatorias"&&g.encerrado){
      const winner=gameWinner(g);
    ...
    await promiseWithTimeout(
      ...
      15000,
      "O salvamento demorou mais que o esperado."
    );

    state.current=c;
    state.currentGame=null;
    toast(clear?"Resultado apagado.":"Resultado salvo.");
    openCompetition(c,true);
  }catch(error){
    console.error("Erro ao salvar resultado:",error);

    if(String(error?.message||"").includes("demorou")){
      toast("A internet está lenta. O resultado não foi confirmado; tente novamente.");
    }else{
      toast("Não foi possível salvar o resultado. Tente novamente.");
    }
  }finally{
    savingGame=false;
