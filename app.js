...
async function saveChamp(){load(true);try{
  if(state.wizard.id){
  ...
  delete d.id;
  delete d.playerTeamId;
  d.atualizadoEm=serverTimestamp();

  if(state.wizard.id){
    await updateDoc(doc(db,"escolas",state.school.id,"campeonatos",state.wizard.id),d);
  }else{
    d.criadoEm=serverTimestamp();
    await addDoc(campsRef(),d);
  }

  toast(state.wizard.id?"Campeonato atualizado.":"Campeonato criado.");
    if(state.wizard.id){
...
function renderScorers(c,box){
  ...
  scorers.forEach((player,index)=>{
    // Classificação de competição: 1º, 1º, 3º...
    ...
    row.innerHTML=`
      <strong class="scorer-position">${currentPosition}º</strong>
...
function renderGoalInputs(){
  ...
  const g=state.currentGame;
  const a=c.participantes.find(p=>p.id===g.casaId);
  const b=c.participantes.find(p=>p.id===g.foraId);
  ...
  box.dataset.casa=JSON.stringify(a?.jogadores||[]);
  box.dataset.fora=JSON.stringify(b?.jogadores||[]);
}
...
function updateGoalInputs(){
  ...
  const box=$("golsFutsal");
  const playersHome=JSON.parse(box.dataset.casa||"[]");
  const playersAway=JSON.parse(box.dataset.fora||"[]");
  const scoreHome=Math.max(0,+$("placarCasa").value||0);
  const scoreAway=Math.max(0,+$("placarFora").value||0);
  const homeBox=$("golsCasaInputs");
  const awayBox=$("golsForaInputs");

  // Primeiro preserva tudo que o professor já selecionou na tela.
  let homeValues=currentGoalSelections("casa");
  let awayValues=currentGoalSelections("fora");

  // Na primeira abertura, recupera os autores já salvos na partida.
  if(!homeValues.length)homeValues=savedGoalSelections("casa");
  if(!awayValues.length)awayValues=savedGoalSelections("fora");

  homeBox.innerHTML="";
  awayBox.innerHTML="";

  for(let i=0;i<scoreHome;i++){
    homeBox.appendChild(goalSelect(playersHome,"casa",homeValues[i]||""));
  }

  for(let i=0;i<scoreAway;i++){
    awayBox.appendChild(goalSelect(playersAway,"fora",awayValues[i]||""));
  }
}
...
async function saveGame(clear=false){const c=JSON.parse(JSON.stringify(state.current)),loc=locateGame(c,state.currentGame.id);if(!loc)return toast("Jogo não encontrado.");const g=c[loc.list][loc.i];if(clear){Object.assign(g,{encerrado:false,placarCasa:null,placarFora:null,situacao:"normal",gols:[],sets:[],vencedorPenaltisId:null,vencedorPenaltisNome:"",penaltisCasa:null,penaltisFora:null});}else{g.data=$("dataJogo").value;g.hora=$("horaJogo").value;g.local=$("localJogo").value.trim();g.situacao=$("situacaoJogo").value;if(g.situacao==="adiado"||g.situacao==="cancelado"){g.encerrado=g.situacao==="cancelado";g.placarCasa=null;g.placarFora=null;}else if(c.tipoPlacar==="sets"){if(g.situacao==="normal"){const v=validateSets(c);if(!v.ok)return toast(v.msg);g.sets=state.setDraft;g.placarCasa=v.winsCasa;g.placarFora=v.winsFora;}else{g.sets=[];g.placarCasa=g.situacao==="woCasa"?3:0;g.placarFora=g.situacao==="woFora"?3:0;}g.encerrado=true;}else{let pc=Math.max(0,+$("placarCasa").value||0),pf=Math.max(0,+$("placarFora").value||0);if(g.situacao==="woCasa"){pc=3;pf=0}if(g.situacao==="woFora"){pc=0;pf=3}if(g.situacao==="woDuplo"){pc=0;pf=0}if(loc.list==="eliminatorias"&&g.situacao==="normal"&&pc===pf){
  ...
  g.penaltisCasa=null;
  g.penaltisFora=null;
}g.placarCasa=pc;g.placarFora=pf;g.encerrado=true;g.gols=[];if(["Futsal","Handebol"].includes(c.esporte)&&g.situacao==="normal")document.querySelectorAll("#golsFutsal select").forEach(s=>{if(s.value)g.gols.push({jogadorId:s.value,lado:s.dataset.side});});}}
recalcScorers(c);
