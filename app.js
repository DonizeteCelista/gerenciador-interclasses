...
function bracketCodeCandidates(name){
  const clean=normalizeBracketName(name);
  ...
  if(words.length>1){
    ...
    add((words[0]?.[0]||"")+(words.at(-1)?.slice(0,2)||""));
  }

  for(let first=0;first<letters.length;first++){
    for(let second=first+1;second<letters.length;second++){
      for(let third=second+1;third<letters.length;third++){
        add(letters[first]+letters[second]+letters[third]);
      }
    }
  }

  return candidates.length?candidates:[(letters+"XXX").slice(0,3)];
}

function bracketCodeMap(c){
  const used=new Set();
  const map={};

  (c.participantes||[]).forEach((participant,index)=>{
    const candidates=bracketCodeCandidates(participant.nome);
    ...
    used.add(code);
    map[String(participant.id)]=code;
    map[normalizeBracketName(participant.nome)]=code;
  });

  return map;
}

function bracketName(c,id,name){
  const clean=normalizeBracketName(name);
  if(!clean||clean==="A DEFINIR")return"???";

  const map=bracketCodeMap(c);
  return map[String(id)]||map[clean]||bracketCodeCandidates(name)[0]||"???";
}
      ...
      .forEach(g=>{
        ...
        card.innerHTML=`
          ...
          <div class="bracket-team ${winner?.id===g.casaId?"winner":g.encerrado&&winner&&!direct?"loser":""}">
            <span>${esc(bracketName(c,g.casaId,g.casa))}</span><strong class="bracket-set-scores">${!direct?bracketSetScoreCells(g,"casa"):""}</strong>
          </div>
          <div class="bracket-team ${winner?.id===g.foraId?"winner":g.encerrado&&winner&&!direct?"loser":""}">
            <span>${esc(bracketName(c,g.foraId,g.fora))}</span><strong class="bracket-set-scores">${!direct?bracketSetScoreCells(g,"fora"):""}</strong>
          </div>
