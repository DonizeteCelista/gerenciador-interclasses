...
function show(id){screens.forEach(s=>s.classList.toggle("active",s.id===id));state.screen=id;$("btnVoltar").classList.toggle("hidden",id==="telaBusca");updateActiveDevicesWidget();sendPresenceHeartbeat();scrollTo({top:0,behavior:"smooth"});}
...
function devicePresenceId(){
  // Identificador exclusivo desta sessão aberta. sessionStorage evita que cópias,
  // restaurações ou navegadores clonados compartilhem o mesmo registro.
  let id=sessionStorage.getItem("interclasses-presence-session-v4");
  if(!id){
    if(globalThis.crypto?.randomUUID){
      ...
      id=`sess-${Date.now().toString(36)}-${randomPart}`;
    }
    sessionStorage.setItem("interclasses-presence-session-v4",id);
  }
  return id;
}

function presenceSchoolRef(){
  return doc(db,"escolas",state.school.id);
}

function updateActiveDevicesWidget(){
  const widget=$("activeDevicesWidget");
  if(!widget)return;

  const visible=!!state.admin&&!!state.school;
  widget.classList.toggle("hidden",!visible);
  if(!visible)return;

  const cutoff=Date.now()-120000;
  const total=Object.values(state.presenceDocs||{}).filter(item=>
    item&&item.administrador!==true&&Number(item.ultimoSinal||0)>=cutoff
  ).length;
  $("activeDevicesCount").textContent=String(total);
}

async function sendPresenceHeartbeat(forceInactive=false){
  if(!state.school?.id)return;
  const dispositivo=devicePresenceId();
  const field=`presencas.${dispositivo}`;

  try{
    // Administradores nunca entram na contagem. Caso o aparelho tenha aberto a página
    // anteriormente como aluno, o registro antigo também é removido.
    if(forceInactive||state.admin){
      await updateDoc(presenceSchoolRef(),{[field]:deleteField()});
      return;
    }

    await setDoc(presenceSchoolRef(),{
      presencas:{
        [dispositivo]:{
    ...
    console.warn("Não foi possível registrar dispositivo ativo:",error);
  }
}

function bindPresenceListeners(){
  if(state.presenceListenersBound)return;
  state.presenceListenersBound=true;
  ...
  // pagehide não aguarda operações do Firestore com segurança. O registro expira
  // sozinho após 90 segundos caso a página seja fechada.
}

function stopPresence(){
  if(state.presenceUnsub){state.presenceUnsub();state.presenceUnsub=null;}
  clearInterval(state.presenceHeartbeat);
  clearInterval(state.presenceRefresh);
  state.presenceHeartbeat=null;
  state.presenceRefresh=null;
  state.presenceDocs={};
  updateActiveDevicesWidget();
}
...
function startPresence(){
  ...
  sendPresenceHeartbeat();
  state.presenceHeartbeat=setInterval(()=>sendPresenceHeartbeat(),6000);
  state.presenceRefresh=setInterval(updateActiveDevicesWidget,2500);

  // Usa o próprio documento da escola, evitando depender de regras extras
  // para uma subcoleção de presença.
  state.presenceUnsub=onSnapshot(presenceSchoolRef(),snap=>{
    state.presenceDocs=snap.exists()?(snap.data().presencas||{}):{};
    updateActiveDevicesWidget();
  },error=>{
    console.warn("Contagem de dispositivos indisponível:",error);
  });
}
...
function openReorganizer(){
  ...
  state.reorderTab=state.current.formato==="grupos"?"grupos":"ordem";
...
function renderReorganizer(){
  ...
  if(state.reorderTab==="grupos")renderReorderGroups(box);
...
if($("btnCopiarPix"))$("btnCopiarPix").onclick=copyPix;
if($("btnCopiarPixModal"))$("btnCopiarPixModal").onclick=copyPix;
if($("btnFecharApoio"))$("btnFecharApoio").onclick=closeSupportModal;
if($("modalApoio"))$("modalApoio").onclick=e=>{if(e.target.id==="modalApoio")closeSupportModal();};
if($("modalApoio"))showSupportModal();
